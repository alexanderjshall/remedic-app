...
*/
export async function getTranslatedText(text: string, sourceLangCode:string, targetLangCode: string): Promise<string> {
...
export async function getTranslatedTexts(text: string[], sourceLangCode:string, targetLangCode: string): Promise<string> {
  const query: TranslationQuery = {
    q: text,
    source: sourceLangCode,
    target: targetLangCode,
  ...
  } else return Promise.reject(new Error(`Something Has Gone Wrong: ${errors}`));
}
