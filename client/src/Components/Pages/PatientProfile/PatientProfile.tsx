...
function PatientProfile() {
  const { getTranslatedText, patientInfo, updatePatient } = useContext(
    PatientContext
  )!;
  ...
  const handleChangePicture = (files: FileList | null) => {
    if (files) {
      ...
      if (!newPicture.type.startsWith("image/")) {
        return;
      }
          ...
          {isPictureSaved ? (
            <label className="border-invisible rounded-md px-4 py-1 ml-2 bg-blue-superlight cursor-pointer">
              ...
              <input
                ...
                className="hidden"
                onChange={(e) => {
                  handleChangePicture(e.currentTarget.files);
                }}
              />
            </label>
          ) : (
            <div>
              ...
              <button
                className="border-invisible rounded-md px-4 py-1 bg-red-negative text-white ml-2"
