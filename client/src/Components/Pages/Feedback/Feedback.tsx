...
const Feedback = () => {
  ...
  const [stars, setStars] = useState(Array<RatingStar>(5).fill({ filled: false}));

  const history = useHistory();


  const handleStarClick = (e: React.MouseEvent<HTMLImageElement, MouseEvent>, i: number): void => {
  ...
  //TODO fill this in as necessary
  ...
  return (
    <div className="flex items-center justify-center flex-col bg-white h-full">
      <div className="flex items-center justify-center flex-col h-5/6 w-5/6 shadow-lg">
        <img src={logoReduced} alt="logo" className="w-32 mb-10 animate-pulse"/>
      ...
      <footer className="bg-blue-dark w-100 h-4">
